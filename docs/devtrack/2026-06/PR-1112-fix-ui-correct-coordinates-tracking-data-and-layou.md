...
5.  **Implement Marker Hover Delay:**
    *   In `apps/web/app/[locale]/map/PharmacyMap.tsx`, within the marker creation `useEffect`, modify the `mouseout` handler:
        ...
        marker.on("mouseout", () => {
            setTimeout(() => {
                if (!marker.isPopupOpen()) { // Check if popup is still open before removing hover class
...
```json
{"name":"SahiDawa API","description":"India's Open-Source Citizen Medicine Verifier & Rural Health Bridge","version":"1.0.0","status":"running","environment":"development","endpoints":{"health":"/health","docs":"/api/docs"},"repository":"https://github.com/RatLoopz/sahidawa-india","timestamp":"2026-06-02T08:49:27.706Z"}
```
